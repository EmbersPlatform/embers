defmodule EmbersWeb.Web.Moderation.RoleController do
  @moduledoc false
  ...
  def update(conn, %{"role_id" => role_id} = params) do
    with \
      role when not is_nil(role) <- Roles.get_by_id(role_id),
      name = Map.get(params, "name", role.name),
      permissions = Map.get(params, "permissions", role.permissions),
      {:ok, role} <- Roles.update(role.name, %{name: name, permissions: permissions})
    do
