<template>
  <article v-if="show" class="card" :class="{'nsfw': post.nsfw, 'locked' : locked}" :data-card-size="size" focusable tabindex="-1">
    <div class="card-flags" v-if="post.nsfw">
      <div v-if="post.nsfw" class="flag nsfw" :class="{'isLocked' : locked}">
        <p>
          <strong data-tip="Contiene material explicito." data-tip-position="left" data-tip-text>NSFW:</strong>
        </p>
        <button data-button-font="little" data-button-size="little" class="button" @click.prevent="toggleLock">
          {{(!this.locked) ? 'ocultar' : 'ver'}}
        </button>
      </div>
    </div>
    <div class="card-wrapper" :class="{'locked' : locked, 'is-picker': isPicker}">
      ...
      <section class="card-wrapper-content" :class="{'big-text': (post.body && bigTextBody)}">
        ...
        <div v-if="post.body && bodyTooLong && !unfolded" class="controls">
          <button v-if="size != 'little'" @click.prevent="unfold" class="button" data-button-size="medium" data-button-font="medium" data-button-normal-text>
            Leer todo
          </button>
          <router-link v-else :to="post.url" class="button" data-button-size="medium" data-button-font="medium" data-button-normal-text>
            Ir al post
          </router-link>
        </div>
        ...
        <div class="card-wrapper-content-tags" v-if="post.tags && post.tags.length">
          <router-link :to="`/search/in:${tag.name}`" class="tag" v-for="tag in post.tags" :key="tag.id">
            #{{ tag.name }}
          </router-link>
        </div>
      </section>
    ...
    <footer v-if="!isPostView || isPostView && (!isOwner || hasReactions || post.stats.comments > 0 || post.stats.shares > 0)" class="actions">
      ...
      <ul class="actions-panel">
        <li v-if="loggedUser && !isOwner" class="emoji-pick-list">
          <span ref="trg_picker" class="picker-switch" :class="{'open' : isPicker}" data-tip="Elegir una reaccion" data-tip-position="bottom" data-tip-text>
            <svgicon name="s_new-reaction" class="emoji"></svgicon>
          </span>
          <ul v-if="isPicker" data-name="reacciones" ref="picker">
            <li v-for="reaction in reactions" class="reaction" @click="react(reaction)" :key="reaction.id">
              <img :src="`/img/emoji/${reaction}.svg`" :alt="reaction" class="emoji">
            </li>
          </ul>
        </li>
        <li v-if="loggedUser">
          <span @click="toggleFav" :data-tip="(post.isFaved) ? 'Quitar de mis favoritos' : 'Agregar a favoritos'" data-tip-position="bottom" data-tip-text>
        ...
        <li v-if="loggedUser && (!isOwner || isOwner && post.stats.shares > 0)">
          <span @click="share" :data-tip="(isOwner) ? 'No puedes compartir algo tuyo' : 'Compartir post'" data-tip-position="bottom" data-tip-text>
