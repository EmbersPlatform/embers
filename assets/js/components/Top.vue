<template>
	<nav id="top">
		<ul id="breadcrumbs" v-html="breadcrumbs"></ul>
		<input id="search" type="text" placeholder="Buscar en Embers" v-model="searchParams" @keyup.enter="search">
	</nav>
</template>
<script>
	export default {
		...
		computed:{
			...
			breadcrumbs(){
				...
				for(var i = 0; i < this.url_array.length; i++){
					...
					crumbs += this.url_array[i]+'">'+this.url_array[i].replace('@', '')+'</a></li>'
				}
				return crumbs;
			}
		},
