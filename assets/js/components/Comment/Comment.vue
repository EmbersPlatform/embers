	...
	<div class="comment">
		<header class="header">
			...
			<div class="header-content">
				<h4>
					<router-link class="username" :to="`/@${comment.user.name}`" :data-badge="`${comment.user.badges[0]}`">
						{{comment.user.name}}
					</router-link>
				...
				<footer class="actions">
					<ul class="actions-reactions">
						<li v-for="(meta, reaction) in comment.stats.reactions" @click="react(reaction)" class="reaction" :key="reaction.id" :reacted="meta.reacted">
					...
					<ul v-if="loggedUser" class="actions-panel">
						<li>
							<span @click.prevent="reply">
								Responder&nbsp;<svgicon name="s_reply" class="emoji"></svgicon>
							</span>
						</li>
...
<script>
	...
	export default {
		...
		methods: {
			reply() {
				this.$root.$emit('reply comment', this.comment.user.name);
			},
			/**
				...
				this.$modal.show('dialog', {
		          ...
		          buttons: [
		            ...
		            { title: 'Eliminar',
		              ...
		              class: 'button danger',
		              handler: () => {
		                comment.deleteComment(this.postId, commentObject.id).then(() => this.$emit('deleted', commentObject));
			 ...
			 */
			reactionsDetails() {
				comment.getReactionsDetails(this.comment.id)
					.then(res => {
			...
			react(reaction) {
				...
				if(this.comment.stats.reactions[reaction] && this.comment.stats.reactions[reaction].reacted) {
