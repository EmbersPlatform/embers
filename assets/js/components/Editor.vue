<template>
  <div class="editor">
    ...
    <div class="canvas" :data-title="canvasTitle">
      <textarea class="markup" @input="e => body=e.target.value" :value="body" ref="textarea" data-autoresize v-show="!preview" :placeholder="placeholder" @keydown="isKey" @paste='onPaste' @focus="onFocus"></textarea>
  ...
  export default {
    ...
    computed: {
      ...
      placeholder() {
        ...
        return (!this.attachment) ? placeholder+'...' : 'Si quieres, puedes añadir una descripción...';
      },
      formattedBody() {
    ...
    created() {
      ...
      this.$root.$on(this._uid+'-insert-image', url => {
        ...
        if(url && url !== '') {
          this.body = textarea.value.substring(0, textarea.selectionStart) + '![](' + url + ')' + textarea.value.substring(textarea.selectionEnd, textarea.value.length);
        }
      });
    }
  }
