...
export default class extends BaseController {
  ...
  connect() {
    ...
    Channel.subscribe(
      `post:${this.element.dataset.id}`,
      "reactions_updated",
      ({ added, removed }) => {
        if (added) this._add_reactions(added);
        if (removed) this._remove_reactions(removed);
      }
  ...
  has_reaction(reaction) {
    const reactions = Array.from(
      this.get_target("reactions").children
    ) as HTMLElement[];
    return !!reactions.find(
      (x) => x.dataset.name == reaction && x.hasAttribute("reacted")
    );
  }
  ...
  onToggleReaction(event) {
    ...
    else this.add_reaction(target.dataset.name);
  }
    ...
    for (let name of names) {
      ...
      if (!reaction) {
        ...
        reaction.querySelector(
          ".reaction-total"
        ).textContent = new_total.toString();
      }
    }
  }
