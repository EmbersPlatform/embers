	...
	export default {
		...
		methods: {
			...
			submitMessage(data) {
				...
				chatAPI.sendMessage(this.conversation.user.id, {content: content})
				.then(message => {
		...
		created() {
			...
			.then(conversations => {
				if(this.$route.params.id) {
					let conversation = _.find(conversations, (o) => { return o.user.id == this.$route.params.id});
					if(conversation) {
						this.$store.dispatch('chat/updateConversation', conversation);
					}
				}
		...
		sockets: {
			new_chat_message(data) {
				if(this.conversation && data.conversation_id !== this.conversation.id) {
					this.$store.dispatch('chat/newConversationMessage', data.conversation_id);
				}
			}
		}
	}
