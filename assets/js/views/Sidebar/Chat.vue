		...
		<li class="nav_ nav-grow">
			<ul :class="{'renderbox' : loading}">
				<li v-for="conversation in conversations" class="n_item" :key="conversation.id">
					<span @click="conversationSelected(conversation)" active-class="active" class="n_i_wrap" :data-count="[conversation.unread <= 9 ? conversation.unread : '9+']">
						<avatar :avatar="conversation.user.avatar.small"></avatar>
						<span class="n_i_w_content u_name" :data-badge="`${conversation.user.badges[0]}`">{{conversation.user.name}}</span>
					</span>
				</li>
			</ul>
		</li>
