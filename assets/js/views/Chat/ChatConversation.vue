<template>
	<div id="content" data-layout-type="chat" ref="conversationDiv" :class="{'renderbox' : isLoading}">
		<template v-if="!isLoading">
			...
			<div class="message" v-for="messages in messageBlocks">
				...
				<div class="message-content">
					...
					<p v-html="formatMessage(message.content)" v-for="message in messages"></p>
				</div>
			</div>
		</template>
	</div>
...
<script>
	...
	export default {
			...
			messageBlocks() {
				...
				for(var key in messages) {
					...
					if(typeof messages[key - 1] !== 'undefined') {
						// Get the timestamps
						let currentMessageTimestamp = this.$moment(messages[key].created_at).format("X");
						let previousMessageTimestamp = this.$moment(messages[key - 1].created_at).format("X");

						// If 5 minutes elapsed from last message, message is new enough to be put in a separate block
						tooNewMessage = currentMessageTimestamp-3000 >= previousMessageTimestamp;
					}
		...
		methods: {
			fetch() {
				this.isLoading = true;
				chatAPI.getMessages(this.partner.id, { markAsRead: true})
					.then(res => {
						if(res.items.length) {
							this.oldestId = res.items[res.items.length - 1].id;
							this.$store.dispatch('chat/updateMessages', res.items);
							this.$store.dispatch('chat/updateUnreadMessagesCount', this.unread_messages - this.conversation.unread);
			...
			loadMore() {
				...
				chatAPI.getMessages(this.partner.id, {
		...
		sockets: {
			new_chat_message(data) {
				if(data.from == this.partner.name) {
					...
					chatAPI.markAsRead(this.partner.id);
				}
			}
		}
	}
