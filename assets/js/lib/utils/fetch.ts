...
export const FetchResults = {
  Success: <S, E>(value?: S): NetResult<S, E> => ({
    tag: "Success",
    value: value,
  }),
...
const perform_fetch = async (
  ...
  method: string,
  options: Object = {}
): Promise<NetResult<Response, Response>> => {
  ...
  if (options["params"]) {
    for (let key in options["params"]) {
      if (options["params"][key])
        // Ignore non truthy params like null or undefined. Possible bugs here?
...
export const parse_pagination = async (
  response: Response,
  ...
  const body = as_json ? await response.json() : await response.text();
  return {
    body,
